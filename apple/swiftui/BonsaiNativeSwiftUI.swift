...
private final class BonsaiNativeFrameProbe: NSObject {
  ...
  @objc private func tick(_ link: CADisplayLink) {
    ...
    fputs(
      String(
        format:
          "[BonsaiNativeScrollPerf] frame_report seconds=%.2f frames=%d max_delta_ms=%.2f over_16ms=%d over_33ms=%d over_50ms=%d\n",
        now - reportStartedAt,
        frames,
        maxDeltaMs,
        over16ms,
...
private final class BonsaiNativeHostModel: ObservableObject {
  ...
  func sendClick(_ eventId: Int32?, animation: Animation? = nil) {
    ...
    dispatchEvent(animation: animation) { [callback] in
  ...
  func sendChange(_ eventId: Int32?, text: String, animation: Animation? = nil) {
    guard let eventId else { return }
    dispatchEvent(animation: animation) { [callback, text] in
  ...
  private func dispatchEvent(animation: Animation? = nil, _ emit: @escaping () -> Void) {
    let performEmit = {
      if let animation {
...
private final class BonsaiNativeLazyRowDetachScheduler {
  static let shared = BonsaiNativeLazyRowDetachScheduler()
...
private struct BonsaiNativeLazyListMovePreview: Equatable {
  let fromIndex: Int
  let toOffset: Int
  let rowCount: Int

  func sourceIndex(for position: Int) -> Int {
    guard rowCount > 0 else { return position }
    let fromIndex = min(max(0, fromIndex), rowCount - 1)
    let toOffset = min(max(0, toOffset), rowCount)
    if fromIndex < toOffset {
      if position < fromIndex { return position }
      if position < toOffset - 1 { return position + 1 }
      if position == toOffset - 1 { return fromIndex }
      return position
    }
    if fromIndex > toOffset {
      if position < toOffset { return position }
      if position == toOffset { return fromIndex }
      if position <= fromIndex { return position - 1 }
      return position
    }
    return position
  }
...
private struct BonsaiNativeLazyListRowSlots: RandomAccessCollection {
  typealias Index = Int
  typealias Element = BonsaiNativeLazyListRowSlot

  private let rows: [BonsaiNativeLazyListRowSlot]

  var startIndex: Int { 0 }
  var endIndex: Int { rows.count }

  init(
    count: Int,
    providerId: Int32,
    owner: BonsaiNativeNode,
    movePreview: BonsaiNativeLazyListMovePreview?
  ) {
    if count <= 0 {
      rows = []
      return
    }
    rows = (0..<count).map { position in
      let sourceIndex = movePreview?.sourceIndex(for: position) ?? position
      return BonsaiNativeLazyListRowSlot(
        position: position,
        sourceIndex: sourceIndex,
        key: bonsaiNativeLazyRowKey(owner: owner, providerId: providerId, index: sourceIndex),
        refreshGeneration: owner.lazyListInvalidatedIndices.contains(sourceIndex)
          ? owner.lazyListVersion
          : 0
      )
    }
  }
...
private struct BonsaiNativeNodeView: View {
  ...
  @State private var lazyListMovePreview: BonsaiNativeLazyListMovePreview?
  ...
  private func nativeList(_ proxy: ScrollViewProxy) -> some View {
    ...
    .background {
      ...
      BonsaiNativeScrollViewRegistrationView(listID: node.id)
        .frame(width: 0, height: 0)
    }
    .onAppear {
      listAppeared(proxy)
    ...
    .onChange(of: node.children.count + node.lazyListRowCount) { _, _ in
      if lazyListMovePreview != nil {
        BonsaiNativeListVirtualizationProbe.shared.debug(
          "lazy_move_preview_clear reason=row_count list=\(node.id.uuidString)"
        )
      }
      lazyListMovePreview = nil
      listChildrenCountChanged()
    }
    .onChange(of: node.lazyListVersion) { oldVersion, newVersion in
      if lazyListMovePreview != nil {
        BonsaiNativeListVirtualizationProbe.shared.debug(
          "lazy_move_preview_clear reason=version list=\(node.id.uuidString) old_version=\(oldVersion) new_version=\(newVersion)"
        )
      }
      lazyListMovePreview = nil
    }
    .onChange(of: lazyListMovePreview) { oldPreview, newPreview in
      BonsaiNativeListVirtualizationProbe.shared.debug(
        "lazy_move_preview_state_changed list=\(node.id.uuidString) old=\(describeMovePreview(oldPreview)) new=\(describeMovePreview(newPreview)) version=\(node.lazyListVersion)"
      )
    }
    ...
    if let providerId = node.lazyListProviderId {
      ...
      ForEach(slots) { slot in
        lazyListRow(
          ...
          key: slot.key,
          refreshGeneration: slot.refreshGeneration
        )
      }
        .onDelete { offsets in
  ...
  private func lazyListRows(providerId: Int32) -> some View {
    ForEach(0..<node.lazyListRowCount, id: \.self) { index in
      lazyListRow(
        providerId: providerId,
        index: index,
        key: bonsaiNativeLazyRowKey(owner: node, providerId: providerId, index: index),
  ...
  private func moveLazyListRows(source: IndexSet, destination: Int) {
    ...
    lazyListMovePreview =
      BonsaiNativeLazyListMovePreview(
        fromIndex: fromIndex,
        toOffset: toOffset,
        rowCount: node.lazyListRowCount
      )
    commitLazyListMove(fromIndex: fromIndex, toOffset: toOffset)
  }
  ...
  private func commitLazyListMove(fromIndex: Int, toOffset: Int) {
    ...
    BonsaiNativeListVirtualizationProbe.shared.debug(
      "lazy_move_commit_scheduled list=\(node.id.uuidString) from_index=\(fromIndex) to_offset=\(toOffset) version=\(node.lazyListVersion)"
    )
    BonsaiNativeRenderedFrameScheduler.shared.runAfterRenderedFrame { [model] in
      BonsaiNativeListVirtualizationProbe.shared.debug(
        "lazy_move_commit_emit from_index=\(fromIndex) to_offset=\(toOffset)"
      )
      model.sendChange(eventId, text: "\(fromIndex):\(toOffset)")
      BonsaiNativeListVirtualizationProbe.shared.operationFinished(
        name: "lazy_move",
        listID: node.id,
        startedAt: startedAt,
  ...
  private func describeMovePreview(_ preview: BonsaiNativeLazyListMovePreview?) -> String {
    guard let preview else { return "nil" }
    return "from=\(preview.fromIndex) to=\(preview.toOffset) rows=\(preview.rowCount)"
  }

  private func logVisibleLazySlots(_ slots: BonsaiNativeLazyListRowSlots) {
    guard lazyListMovePreview != nil else { return }
    let limit = min(slots.count, 12)
    let mapping = (0..<limit)
      .map { position in "\(position):\(slots[position].sourceIndex)" }
      .joined(separator: ",")
    BonsaiNativeListVirtualizationProbe.shared.debug(
      "lazy_move_preview_slots list=\(node.id.uuidString) version=\(node.lazyListVersion) slots=\(mapping)"
    )
  }
  ...
  private func scrollFocusedRow(_ proxy: ScrollViewProxy) {
    ...
    if node.lazyListProviderId != nil {
      ...
      guard let providerId = node.lazyListProviderId else { return }
      targetID = AnyHashable(bonsaiNativeLazyRowKey(owner: node, providerId: providerId, index: index))
    } else {
    ...
    BonsaiNativeScrollStressProbe.shared.start(
      ...
      scrollToIndex: { index in
        ...
        if node.lazyListProviderId != nil {
          guard let providerId = node.lazyListProviderId else { return }
          proxy.scrollTo(
            bonsaiNativeLazyRowKey(owner: node, providerId: providerId, index: index),
            anchor: .top
          )
...
private struct BonsaiNativeLazyListRowView: View {
  ...
  @State private var focusedDisappearToken: UUID?
  ...
  private var cachedRenderedChild: BonsaiNativeNode? {
    guard owner.lazyListRowKeyByIndex[index] == key else { return nil }
    return owner.lazyListRowsByIndex[index]
  }
  ...
  private func refreshRow() {
    ...
    guard let renderCallback = bonsaiNativeLazyRowRenderCallback else {
      BonsaiNativeListVirtualizationProbe.shared.debug(
        "lazy_row_refresh_missing_callback provider=\(providerId) index=\(index)"
      )
      return
    }
    let renderStartedAt = CACurrentMediaTime()
    guard let pointer = renderCallback(providerId, Int32(index)) else {
      BonsaiNativeListVirtualizationProbe.shared.debug(
        "lazy_row_refresh_nil_pointer provider=\(providerId) index=\(index)"
      )
      return
    }
    let renderElapsedMs = (CACurrentMediaTime() - renderStartedAt) * 1000
    guard let rendered = nativeNode(from: pointer) else {
      BonsaiNativeListVirtualizationProbe.shared.debug(
        "lazy_row_refresh_missing_node provider=\(providerId) index=\(index)"
      )
      return
    }
    owner.lazyListRowsByIndex[index] = rendered
  ...
  private func scheduleFocusedRowDisappear() {
    ...
    let token = UUID()
    focusedDisappearToken = token
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) {
      guard focusedDisappearToken == token else { return }
      guard !isVisible else { return }
      guard owner.listFocusedRowIndex == index else { return }
  ...
  private func scheduleRelease() {
    guard let rendered = cachedRenderedChild else { return }
    let token = UUID()
    releaseToken = token
    BonsaiNativeLazyRowDetachScheduler.shared.schedule {
      guard releaseToken == token else { return }
      guard !isVisible else { return }
      guard cachedRenderedChild === rendered else { return }
      releaseToken = nil
    }
  }
...
public func bonsai_native_swiftui_set_lazy_list_rows(
  ...
  let staleIndices = Set(node.lazyListRowsByIndex.keys.filter { index in
  ...
  for index in staleIdentityKeyIndices {
    node.lazyListIdentityKeyByIndex[index] = nil
  }
